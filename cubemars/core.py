...
class AsyncMotor:
    ...
    def __init__(self, bus: BusABC, motor_id: int):
        self.bus = bus
    ...
    def process_message(self, msg: can.Message):
        ...
        if msg.arbitration_id & 0xFF == self.motor_id:
            self._feedback = unpack_motor_feedback(bytes(msg.data))
    ...
    async def _monitor_loop(self):
        ...
        try:
            ...
            logger.warning(
                f"Motor {self.motor_id}: Could not attach listener (Bus shared?). Feedback will be disabled."
            )
            return
    ...
    async def _send_command(self, mode: CanPacketId, *args):
        ...
        for attempt in range(retries):
            try:
                ...
                # If it's a "no buffer space" error, wait and retry.
                # This is common if we are sending faster than the bus can handle.
                if "No buffer space available" in str(e):
                    if attempt < retries - 1:
                        await asyncio.sleep(0.005)  # Wait 5ms before next attempt
                        continue
                # For other errors or if retries are exhausted, log and raise.
                logger.error(
                    f"Failed to send CAN message after {attempt + 1} attempts: {e}"
                )
                raise
